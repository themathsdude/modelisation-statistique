---
title: Le nombres de téléphones, un nouvel indicateur de développement?
...
Nous allons vérifier que nos observations sont de "bonnes qualités" en cherchant si nous avons des valeurs aberrantes, des points leviers ou des distance de Cook trop élevées. Commençons par les valeurs aberrantes et les points leviers.
...
Nous avons centré et réduit nos observations donc nos coefficients $\beta_j$ sont comparables. Dans notre modèle linéaire, c'est le PIB du pays qui influence le plus $T$. La part de la population dans les services influence également positivement $T$, même si cette influence est moindre. Finalement, le taux de natalité du pays influence également $T$ mais cette fois-ci négativement.
...
On peut donc conclure que le nombre de téléphones pour mille habitants est liés linéairement à des caractéristiques préalablement existantes. Ce n'est donc pas un critère de développement qui bouscule les classifications déjà établies. 
 
Terminons avec un peu de cross-validation, en découpant aléatoirement notre jeu de données en 80/20 :
...
prediction <- predict.lm(mod_affiné, newdata = df.countries.test)
...
On voit que la plupart des valeurs tombent dans leurs intervalles de confiance. Cependant, parfois certaines valeurs sont très loin (selon le tirage aléatoire des 80/20). Peut-être qu'il nous faudrait un modèle plus affiné par région d'appartenance du pays ?

<br>

## III - Modélisation linéaire avec variable qualitative

### A - Influence de la région et ANOVA
...
La variable $T$ engendre-t-elle la distinction traditionnelle pays du sud / pays du nord? 


Nous avons observé dans l'analyse bi-variée que la région d'appartenance du pays influe sur $T$ (cf. Box-plot par région). Nous allons ici voir dans quelle mesure cette influence à lieu. 
...
boxplot_region
```
La variable Région possède 11 niveaux avec des tailles différentes. Notre plan d'expérience est complet, répété et n'est pas équilibrée en chaque variable. 
...
Nous avons `r nb_aberrant3` valeurs aberrantes, soit `r round((nb_aberrant3/n)*100,2)` % de nos observations ne sont pas dans l'intervalle [-2;2] ce qui est acceptable (moins de 5%). Nous avons `r nb_leviers3` points leviers, soit un nombre considérable.  Analysons à présent les distances de Cook. 
...
Pour conclure, la région du pays d'appartenance joue un rôle quand au nombre de téléphones pour mille habitants. Les coefficients pour l'Amérique du Nord et l'Europe de l'ouest se détache positivement. Inversement, ce sont les pays d'Afrique sub-saharienne qui ont le taux le plus bas, suivis par les pays d'Afrique du Nord.  
<br>



### C - Pays du nord et pays du sud

Dans notre régression précédente, nous avions des modalités avec une p-valeur associée suffisamment faible pour avoir des coefficients significatifs. Ce n'était pas le cas pour d'autres modalités, notamment la région Baltique, l'Afrique du nord ou l'Océanie. La région Baltique contient les données de seulement 3 pays et l'Afrique du nord de 5. Pour palier à cette problématique, nous décidons de regrouper des régions ensembles, suivant si elles sont dites du nord ou du sud.

Nous créons donc deux nouveau sous-groupes:
 - les régions du nord : l'Amérique du nord et l'Europe de l'ouest.
 - les régions du sud : les 9 autres régions, soit l'ex-URSS, l'Amérique latine et caribéenne, l'Europe de l'est, les Baltiques le proche orient, l'Afrique du nord, l'Océanie, l'Asie et l'Afrique sub-saharienne.
...
Ici, nous créons clairement 2 modalités pour notre variable Région, bien séparées par leurs 1ers et 3èmes quartiles, ce qui nous est confirmé par l'ANOVA :
...
Le box-plot nous suggère même qu'on pourrait prédire la région d'appartenance du pays en fonction du nombre de téléphones à l'aide d'une régressioon logistique. Mais ce n'est pas le sujet de cette analyse.


## Conclusion

  Au fil de cette analyse, nous nous sommes rendus compte que le nombre de téléphones pour mille habitants est corrélé aux indicateurs dits classiques de développement. En effet, en 2000, le PIB, le taux de natalités et la part de la population dans les services suffisaient à prédire cette variable. Ainsi, notre variable d’intérêt n'ajoute pas d'information suffisamment pertinente lors d'une analyse de développement d'un pays. 


  Quand serait-il aujourd'hui? Le nombre de téléphones pour mille habitants a explosé dans tous les pays. En France, 54 millions de personnes en 2020 possédaient un smartphone (chiffre de l'Arcep, l'Autorité de régulation des communications électroniques) pour 67 millions d'habitants (chiffre de l'INSEE). On a donc un taux de 800 téléphones pour mille habitants. Nous n'étions qu'à 586 en 2000. Cette explosion n'a pas eu lieu que dans les pays développés. Les smartphones ont pénétré les marchés émergents au point d'avoisiner des taux rencontrés dans les pays développés. Pourtant, les critères comme le PIB et le taux de natalités ne se sont pas harmonisés au niveau mondial. 


  Avec de nouvelles données, on pourrait se demander si l'explication linéaire du nombre de téléphones pour mille habitants par les critères de développement serait-elle encore pertinente aujourd'hui?
