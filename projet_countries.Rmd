...
mod1 <- lm(Téléphones ~  Densité.population + PIB + Agriculture + Services + Taux.de.natalité, data = df.countries)
summary_mod1 <- summary(mod1)
summary_mod1
```

Analysons ce sommaire. Le $R^2$ vaut `r summary_mod1$r.squared` qui est proche de 1 avec une p-valeur associée très faible, donc on rejette l'hypothèse selon laquelle tous les $\beta_j$ sont nuls pour $j=0,...,5$. Le modèle de régression linéaire permet d'expliquer relativement bien notre variable $T$.
...
Nous allons vérifier que nos variables sont de "bonnes qualités" en cherchant si nous avons des valeurs aberrantes, des points leviers ou des distance de Cook trop élevées. Commençons par les valeurs aberrantes et les points leviers.
...
choix1 <- regsubsets(Téléphones~ Densité.population + PIB + Agriculture + Taux.de.natalité , data = df.countries, nvmax = 5, really.big = T)
par(mfrow = c(1, 2))
...
```{r choix-modèle2}
mod2 <- lm(Téléphones ~ PIB + Services + Taux.de.natalité, data = df.countries)
```

On itère le procédé avec nos 3 variables explicatives restantes.

```{r choix-modèle2, fig.width = 10}
choix2 <- regsubsets(Téléphones~ PIB + Services + Taux.de.natalité , data = df.countries, nvmax=3, really.big=T)
par(mfrow = c(1, 2))
...
nouvcountries <- df.countries[c(-115), ]
...
## III - Modélisation linéaire avec variable qualitative, réalisation d'une ANCOVA. 
...
Réalisons une ANCOVA sur un jeu de donné d'apprentissage comprenant 80% de nos pays étudiés, puis vérifier à l'aide des 20% de pays restants la précision de notre modèle trouvé.
<details>
<summary>Sortie du modèle avec toutes les interactions</summary>

```{r summary-complet}
complet <- lm(Téléphones~ Région + Services + Taux.de.natalité + PIB + Région*PIB + Région*Services + Région*Taux.de.natalité, data= df.countries)
...
```{r mod-sans-int}
mod.sans.int <- lm(Téléphones~ Région + Services + Taux.de.natalité + PIB + Région*PIB + Région*Services, data= df.countries.app)
anova(mod.sans.int)
...
# a voir en fonction des interactions qu'on mets dans lm
...
df.residus <- data.frame(pays = df.countries.app$Pays, residu = rstudent(mod.retenu))
n <- length(df.countries.app$Téléphones)
...
df.countries.app <- df.countries[idx,]
df.countries.test <- df.countries[-idx,]

n_test <- length(df.countries.test$Téléphones)
prediction <- predict.lm(mod.retenu, newdata = df.countries.test)
...
sigma_hat <- sd(df.countries.test$Téléphones)
...
```{r plot-IC-predict}
prediction <- predict.lm(mod.retenu, newdata = df.countries.test, interval = "confidence")
df.ic <- as.data.frame(cbind(pays = df.countries.test$Pays, prediction))
rownames(df.ic) <- NULL
...
Au vu de l'histogramme de $T$, nous pouvons penser que $T$ est un échantillon de loi exponentielle, c'est-à-dire que $T$ est la réalisation d'une variable aléatoire  $X$ suivant une loi exponentielle de paramètre $\lambda$. L'espérance d'une loi exponentielle est $\frac{1}{\lambda}$. La méthode des moments nous propose la moyenne empirique de $T$ comme estimateur sans biais, consistant et asymptotiquement normal de l'espérance.
...
histp <- histp + labs(title="Comparaison histogramme-densité" ) + xlab("Nombres de téléphones") + ylab("")
